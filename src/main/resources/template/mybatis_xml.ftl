...
<mapper namespace="${package_path}.dao.${class_name}Dao">
	...
	<delete id="deleteBy" parameterType="${package_path}.model.${class_name}">
	...
	<update id="update" parameterType="${package_path}.model.${class_name}">
		update ${table_name} 
    ...
    <update id="updateBy" parameterType="${package_path}.model.${class_name}">
        update ${table_name}
        <trim prefix="set" suffixOverrides=",">
		<#list table_column as c><#if (c_index>=1)>
            <if test="${c.nameJ} != null<#if (c.type!='Boolean' && c.type!='Date')> and ${c.nameJ} != ''</#if>">${c.name?upper_case} = ${r"#"}{${c.nameJ}},</if>
